...
abstract class MaestroController extends AbstractController
{
    ...
    public function generarExcel( $arrDatos, $nombre)
    {
        if (count($arrDatos) > 0) {
            ...
            for ($i = 'A'; $j <= sizeof($arrCampos) - 1; $i++) {
                ...
                $spreadsheet->getActiveSheet()->getStyle($i)->getFont()->setName('Arial')->setSize(9);
                $spreadsheet->getActiveSheet()->getStyle($i)->getFont()->setBold(true);
