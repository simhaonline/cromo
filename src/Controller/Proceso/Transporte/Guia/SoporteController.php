...
{
   /**
    * @Route("/pro/transporte/guia/soporte", name="mto_transporte_guia_soporte")
    */    
    public function lista(Request $request)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            if ($form->get('btnFiltrar')->isClicked()) {
                ...
                $query = $this->getDoctrine()->getRepository(Guia::class)->listaSoporte($codigoDespacho);
            }
            if ($form->get('btnSoporte')->isClicked()) {
                $arrGuias = $request->request->get('chkSeleccionar');
                $respuesta = $this->getDoctrine()->getRepository(Guia::class)->soporte($arrGuias);
                $codigoDespacho = $form->get('txtNumero')->getData();
                $query = $this->getDoctrine()->getRepository(Guia::class)->listaSoporte($codigoDespacho);
            }
        }
