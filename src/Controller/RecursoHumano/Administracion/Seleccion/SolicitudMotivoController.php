    ...
    public function lista(Request $request, PaginatorInterface $paginator )
    {
        ...
        if ($form->isSubmitted()) {
            ...
            if ($form->get ('btnExcel')->isClicked ()) {
                $raw['filtros'] = $this->getFiltros ($form);
                General::get ()->setExportar ($em->getRepository (RhuSolicitudMotivo::class)->lista ($raw), "Grupos");
            }
        ...
        $arGrupos = $paginator->paginate ($em->getRepository (RhuSolicitudMotivo::class)->lista ($raw), $request->query->getInt ('page', 1), 30);
    ...
    public function nuevo(Request $request, $id)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            if ($form->get('guardar')->isClicked()) {
                ...
                $em->flush();
                return $this->redirect($this->generateUrl('recursohumano_administracion_nomina_embargojuzgado_detalle', array('id' => $arSeleccionMotivo->getCodigoSeleccionMotivoPK())));
            }
        }
