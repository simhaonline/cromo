...
class ProvisionController extends ControllerListenerGeneral
{
    ...
    /**
    * @Route("recursohumano/movimiento/financierto/provision/provision", name="recursohumano_movimiento_financiero_provision_provision")
    */
    ...
    public function nuevo(Request $request, $id)
    {
        ...
        if ($id != 0) {
            $arProvision = $em->getRepository(RhuAporte::class)->find($id);
			if (!$arProvision) {
                return $this->redirect($this->generateUrl('recursohumano_movimiento_financiero_provision_provision'));
            }
		}else{
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            if ($form->get('guardar')->isClicked()) {
                ...
                $em->flush();
                return $this->redirect($this->generateUrl('recursohumano_movimiento_seguridadsocial_aporte_detalle', ['id' => $arProvision->getCodigoProvisionPk()]));
            }
        }
    ...
    /**
     * @Route("recursohumano/movimiento/financiero/provision/detalle/{id}", name="recursohumano_movimiento_financierto_provision_detalle")
     */
    public function detalle(Request $request, $id)
    {
        ...
        $arProvisionDetalles = $paginator->paginate($em->getRepository(RhuAporteDetalle::class)->lista($id), $request->query->getInt('page', 1), 30);
