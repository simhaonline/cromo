...
class EmpleadoProgramarController extends Controller
{
    ...
    /**
     * @Route("/turno/buscar/empleadoprototipo/{codigoContratoDetalle}", name="turno_buscar_empleadoprototipo")
     */
    public function prototitpo(Request $request, $codigoContratoDetalle = null)
    {
        $session = new Session();
        $em = $this->getDoctrine()->getManager();
        $paginator = $this->get('knp_paginator');
        ...
        $arEmpleados = $paginator->paginate($em->getRepository(RhuEmpleado::class)->turnoBuscarPrototipo(), $request->query->get('page', 1), 20);
        return $this->render('turno/buscar/empleadoPrototipo.html.twig', [
            'arEmpleados' => $arEmpleados,
            'form' => $formFiltro->createView()
        ]);
    }
