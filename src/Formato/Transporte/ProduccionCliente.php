...
class ProduccionCliente extends \FPDF {
    ...
    public function Generar($em, $fechaDesde, $fechaHasta) {
        ...
        $this->Body($pdf);
        $pdf->Output("ProduccionCliente.pdf", 'D');
    }
    ...
    public function EncabezadoDetalles() {
        $this->Ln(12);
        $header = array('ID', 'CLIENTE', 'UNIDADES','FLETE', 'MANEJO', 'TOTAL');
    ...
    public function Body($pdf) {
        $em = BaseDatos::getEm();
        $arProducciones = $em->getRepository(TteGuia::class)->informeProduccionCliente(self::$fechaDesde, self::$fechaHasta)->getQuery()->getResult();
        $pdf->SetX(10);
        $pdf->SetFont('Arial', '', 7);
        $unidades = 0;
        $flete = 0;
        ...
        foreach ($arProducciones as $arProduccion) {
            $pdf->Cell(15, 4, $arProduccion['codigoClienteFk'], 'LRB', 0, 'L');
            $pdf->Cell(60, 4, utf8_decode($arProduccion['clienteNombre']), 'LRB', 0, 'L');
