...
class InvOrdenTipoRepository extends ServiceEntityRepository
{
    ...
    public function  lista($raw)
    {
        ...
        $queryBuilder = $this->getEntityManager()->createQueryBuilder()->from(InvOrdenTipo::class, 'ort')
            ->select('ort.codigoOrdenTipoPk')
            ->addSelect('ort.nombre');


        if ($codigo) {
            $queryBuilder->andWhere("ort.codigoOrdenTipoPk = '{$codigo}'");
        }

        if($nombre){
            $queryBuilder->andWhere("ort.nombre LIKE '%{$nombre}%'");
        }

        $queryBuilder->addOrderBy('ort.codigoOrdenTipoPk', 'DESC');
        ...
        if ($arrDetallesSeleccionados) {
            if (count($arrDetallesSeleccionados)) {
                foreach ($arrDetallesSeleccionados as $codigo) {
                    $arRegistro = $em->getRepository(InvOrdenTipo::class)->find($codigo);
                    if ($arRegistro) {
