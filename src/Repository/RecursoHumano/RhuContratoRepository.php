...
class RhuContratoRepository extends ServiceEntityRepository
{
    ...
    public function lista()
    {
        ...
        $queryBuilder = $this->_em->createQueryBuilder()->from(RhuContrato::class, 'c')
            ...
            ->addSelect('ca.nombreCorto as caja')
            ->addSelect('sa.nombreCorto as salud')
            ->addSelect('pa.nombreCorto as pension')
            ->addSelect('ec.nombreCorto as cesantia')
...
//
    ...
    public function informeContratoFechaRetiro($raw)
    {
        ...
        if ($fechaDesde) {
            $queryBuilder->andWhere("c.fechaHasta >= '{$fechaDesde} 00:00:00'");
        }
        if ($fechaHasta) {
