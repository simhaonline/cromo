...
class TteColorRepository extends ServiceEntityRepository
{
    ...
    public function lista($raw)
    {
        ...
        $queryBuilder = $this->getEntityManager()->createQueryBuilder()->from(TteColor::class, 'c')
            ->select('c.codigoColorPk')
            ->addSelect('c.nombre');

        if ($codigo) {
            $queryBuilder->andWhere("c.codigoColorPk = '{$codigo}'");
        }
        ...
        $queryBuilder->addOrderBy('c.codigoColorPk', 'DESC');
    ...
    public function eliminar($arrSeleccionados)
    {
        ...
        if ($arrSeleccionados) {
            foreach ($arrSeleccionados as $codigo) {
                $arRegistro = $em->getRepository(TteColor::class)->find($codigo);
                if ($arRegistro) {
