...
class TurSoporteContratoRepository extends ServiceEntityRepository
{
    ...
    public function lista($raw, $id)
    {
        ...
        $queryBuilder
            ...
            ->addSelect('sc.horasDescansoReales')
            ->addSelect('sc.horasDiurnasReales')
            ->addSelect('sc.horasNocturnasReales')
            ->addSelect('sc.horasFestivasDiurnasReales')
            ->addSelect('sc.horasFestivasNocturnasReales')
            ->addSelect('sc.horasExtrasFestivasNocturnasReales')
            ->addSelect('sc.horasRecargoReales')
            ->addSelect('sc.horasRecargoNocturnoReales')
    ...
    public function generarHoras($arSoporte, $arSoporteContrato, $arFestivos)
    {
        ...
        for ($i = $intDiaInicial; $i <= $intDiaFinal; $i++) {
                ...
                if ($arProgramacion['dia' . $i] != "") {
                    ...
                    if ($arTurno) {
                        ...
                        $arrTurnos[] = [
                            ...
                            'turnoFijo' => $arSoporteContrato['turnoFijo']
                        ];
            ...
            foreach ($arrTurnos as $arrTurno) {
                ...
                //Diferente de turno fijo para que no entre cuando sea turno fijo
                if ($arrHoras1 && !$arrTurno['turnoFijo']) {
                    $arSoporteHora = new TurSoporteHora();
                    $arSoporteHora->setSoporteRel($arSoporte);
                    $arSoporteHora->setSoporteContratoRel($em->getReference(TurSoporteContrato::class, $arSoporteContrato['codigoSoporteContratoPk']));
                    $arSoporteHora->setAnio($arSoporteContrato['anio']);
...
//            // validacion para turnos complementarios
        ...
        if($arrTurno['turnoFijo']) {
            if($arrTurno['novedad'] == 0 && $arrTurno['complementario'] == 0) {
                $arrHoras = array(
                    'horasDescanso' => 0,
                    'horasNovedad' => 0,
                    'horasDiurnas' => 8,
    ...
    public function distribucion($arSoporte, $arGrupo, $arSoporteContrato, $arrSemanas, $arrSemanasCompensacion, $arrDomingos, $arrSoloFestivos) {
        ...
        if($arSoporteContrato->getCodigoDistribucionFk() == 'SER01') {
            if(!$arSoporteContrato->getTurnoFijo()) {
        ...
        if($arSoporteContrato->getCodigoDistribucionFk() == 'DP003') {
            $dias = $arSoporteContrato->getDiasTransporte();
            $horas = $dias * 8;
            $arSoporteContrato->setHoras($horas);
            $arSoporteContrato->setHorasDescanso(0);
            ...
            $vrAdicionalDevengadoPactado -= $arSoporteContrato->getVrDiurna();
            if($vrAdicionalDevengadoPactado > 0) {
                $vrAdicionalDevengadoPactado -= $arSoporteContrato->getVrAuxilioTransportePago();
                $vrAdicional1 = $arSoporteContrato->getVrAuxilioTransportePago();
                $arSoporteContrato->setVrAdicionalDevengadoPactado($vrAdicionalDevengadoPactado);
                $arSoporteContrato->setVrAdicional1($vrAdicional1);
            } else {
                $arSoporteContrato->setVrAdicionalDevengadoPactado(0);
                $arSoporteContrato->setVrAdicional1(0);
            }
